...
export default {
  ...
  methods: {
    ...
    add_link () {
      ...
      this.restore_selection()
      if (!forceFocus) {
        this.paste_html_at_caret(`<div><div contenteditable="false" style="display: inline"><a href="${this.addLinkURL}" target="_blank">${this.addLinkName}</a></div></div>`, false)
      } else {
        document.getElementById('rich_editor').insertAdjacentHTML('beforeend', `<div><div contenteditable="false" style="display: inline"><a href="${this.addLinkURL}" target="_blank">${this.addLinkName}</a></div></div>`)
      }
      this.update_edited_notes()
