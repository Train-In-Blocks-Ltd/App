...
const Multiselect = () =>
    import(
        /* webpackChunkName: "components.multiselect", webpackPreload: true  */ "./Multiselect.vue"
    );
...
export default class Products extends Vue {
    isNewProductOpen: Boolean = false;
    newProduct: Object = {
      name: String,
      notes: String,
      price: Number,
    ...
    disableCreateProductButton: Boolean = true;
    ...
    currencies: String[] = ['GBP', 'USD', 'JPY', 'AUD', 'CAD'];
    // -----------------------------
    // General
    // -----------------------------
    checkForm () {
     ...
     */
    resolveProductsMultiselect (res: String) {
      switch (res) {
        case 'Delete':
          this.deleteProducts()
          break
     ...
     */
    resolveIfProductChanged (product: Object) {
        ...
        if (String(product.price) > 1) {
          this.updateProduct(product.id)
            ...
            utilsModule.responsePopUpRef?.open({
                ...
                text: "Price must be larger than 1.",
            });
        }
      }
    }
     ...
     * @param {integer} id - The id of the product.
     */
    changeSelectCheckbox (id: Number) {
