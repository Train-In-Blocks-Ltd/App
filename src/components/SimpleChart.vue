  ...
  <card-wrapper class="simple_chart" backgroundColor="var(--fore)" noHover>
    <div class="content_wrapper">
      ...
      >
        ...
        <g class="plot">
          <g v-for="(data, index) in pathValues" :key="'ptVal_' + index">
            <line
              v-if="!isNaN(data[1])"
              :x1="`${data[0]}%`"
              :x2="`${data[2]}%`"
...
export default {
  ...
  methods: {
    ...
    prediction(day) {
      ...
      return ((MAX_Y_VALUE * (90 + (10 - PREDICTED_PROPORTION))) / 90).toFixed(
        this.dataType.type === "Sets" || this.dataType.type === "Reps" ? 0 : 2
      );
    },
      ...
      class DataProcessor {
          ...
          this.yDataset.forEach((data, index) => {
            function relativeInterval(xDataset, index) {
              ...
              const RETURN_INTERVAL_VALUE =
                index !== 0
                  ? (90 / SUM_OF_DATE_DAYS) * sumOfPrevious(xDataset, index)
                  : 5;
        ...
        regression() {
          ...
          const standardDeviation = (dataset, sampleMean) => {
            ...
            return Math.sqrt(
              SUM_OF_SQUARES.reduce((a, b) => a + b) / (dataset.length - 1)
            );
          };
          ...
          for (let i = 0; i < this.scaledXDataset.length; i++) {
            SUM_OF_POINTS.push(
              ((this.scaledXDataset[i] - this.xSampleMean) / this.xSampleSD) *
                ((this.scaledYDataset[i] - this.ySampleMean) / this.ySampleSD)
            );
          }
          this.rCorrelation =
          ...
          const accuracy = () => {
            ...
            for (const index in this.scaledXDataset) {
              SUM_OF_SQUARES.push(
                Math.pow(
                  this.scaledYDataset[index] - y(this.scaledXDataset[index]),
                  2
                )
              );
            }
